...
- 출력: 프로토콜 v0.4 JSON — `decisionBlocks`(주제별 판단)와 `warnings`만. `finalDocumentSections`·`missingSections`는 반환하지 않는다 — 문서는 별도 문서 작성 호출이 결정에서 쓰고, 누락 섹션은 서버가 유도한다(규칙 2b).
...
- 항상 프로토콜 v0.4 JSON만 반환한다. 마크다운·산문·코드펜스 금지.
