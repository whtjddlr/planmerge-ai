...
export function buildMergeNormalizedIdeasPrompt(
  ...
  return [
    ...
    '6a. Every id in that list must be cited by at least one option. An idea you leave out is an opinion dropped from the meeting, which is the one thing this tool exists to prevent. If an idea fits no decision you have made, open a decision for it rather than omitting it.',
    ...
    '10. Every element of decisionBlocks is a decision block: it has sectionKey, topic, selectedOptionId, and an options array. NEVER put an option object (anything with optionType) directly in decisionBlocks — an option belongs inside its own block\'s options array.',
    '11. Gather every option of a block before you write that block. You cannot append an option to a block you have already closed. In measured runs the model closed a block, then found one more alternative for it, placed that alternative in decisionBlocks and stopped there — 20 of 25 ideas were lost that way.',
    '',
    ...
    '',
    'Judgment procedure:',
    '0. First group all ideas by the topic they argue about, so that ideas that disagree about the same thing land in one group. Then write one decision block per group, with every idea in that group as an option of that block. Grouping before writing is what keeps disagreement visible and keeps you from finding a stray option after a block is closed.',
    ...
    '',
    `Valid sourceIdeaIds (all ${normalizedIdeas.length} of them must be cited; use these exact strings, nothing else):`,
    JSON.stringify(normalizedIdeas.map((idea) => idea.id)),
    '',
    `Before you finish: every one of the ${normalizedIdeas.length} ids above appears in some option's sourceIdeaIds, and every element of decisionBlocks is a block with an options array.`,
    '',
    'Return shape:',
    JSON.stringify({
      decisionBlocks: [
